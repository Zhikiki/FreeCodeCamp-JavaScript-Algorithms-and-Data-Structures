...
an object's private variable to the user without the user directly 
...
Remember that C = 5/9 * (F - 32) and F = C * 9.0 / 5 + 32, where F is the 
value of temperature in Fahrenheit, and C is the value of the same temperature 
in Celsius.

Note: When you implement this, you will track the temperature inside the 
class in one scale, either Fahrenheit or Celsius.

This is the power of a getter and a setter. You are creating an API for 
another user, who can get the correct result regardless of which one you 
track.
...
class Thermostat {
  ...
  set temperature(updatedTemperature) {
    this._temperatureFahrenheit = updatedTemperature * 9.0 / 5 + 32;
  }
}
